...
import { SignatureListModal } from "../shared/modal/SignatureListModal";
import { StampListModal } from "../shared/modal/StampListModal";
import { FormOptionsModal } from "../shared/modal/FormOptionsModal";
import { PushButtonOptionsModal } from "../shared/modal/PushButtonOptionsModal";
import { launchImageLibrary } from "react-native-image-picker";
import { Logger } from '../../../util/logger';

type CustomAnnotationCreateRoute = RouteProp<
  AppStackParamList,
...
const CPDFCustomAnnotationCreateExampleScreen = () => {
  ...
  const route = useRoute<CustomAnnotationCreateRoute>();
  ...
  const handleSave = async () => {
    if (pdfReaderRef.current) {
      ...
      if (success) {
        Logger.log("ComPDFKitRN save() : Document saved successfully");
      } else {
        Logger.log("ComPDFKitRN save() : Failed to save document");
      }
    }
  };

  const menuOptions = ["Open Document", "Save"] as const;
  ...
  const onAnnotationCreationPrepared = (
    ...
    Logger.log(`onAnnotationCreationPrepared: ----------->`);
  ...
  const handleLinkSelected = async (annotation: CPDFAnnotation) => {
    if (pdfReaderRef.current) {
      ...
      Logger.log('Updated link annotation action to https://www.compdf.com');
  ...
  const handleSignatureSelected = async (imagePath: string) => {
    if (pdfReaderRef.current) {
      Logger.log("Adding signature to document with path:", imagePath);
  ...
  const handleStampStandardSelected = async (standardStamp: any) => {
    if (pdfReaderRef.current) {
      Logger.log("Preparing standard stamp:", standardStamp);
  ...
  const handleStampCustomSelected = async (imagePath: string) => {
    if (pdfReaderRef.current) {
      Logger.log("Preparing custom stamp from path:", imagePath);
  ...
  const handleImageSelected = async () => {
    ...
    launchImageLibrary(
      ...
      async (res) => {
        ...
        Logger.log("Selected image URI:", uri);
        if (!uri) {
          Logger.log("ComPDFKitRN insertImagePage: invalid image asset");
          return false;
        }
  ...
  const handleFormOptionsConfirm = async (options: CPDFWidgetItem[]) => {
    if (currentWidget) {
      if (options.length === 0) {
        Logger.log('No options provided, aborting update.');
        return;
      }
      Logger.log('Updating form options:');
  ...
  const handlePushButtonConfirm = async (payload: { type: 'url'; url: string } | { type: 'page'; page: number }) => {
    ...
    if (payload.type === 'url') {
      ...
      Logger.log('PushButton action set to URL:', payload.url);
    } else {
      currentPushButton.action = CPDFGoToAction.toPage(payload.page);
      Logger.log('PushButton action set to page:', payload.page);
    }
  ...
  const onViewCreated = () => {
    Logger.log("CPDFReaderView Create ----------------")
    pdfReaderRef.current?.addEventListener(CPDFEvent.FORM_FIELDS_CREATED, (widget) => {
      Logger.log('FORM_FIELDS_CREATED event received for widget:');
          ...
          if (widget instanceof CPDFPushbuttonWidget) {
            ...
            (async () => {
              try {
                ...
                Logger.warn('Failed to get page count for push button', error);
