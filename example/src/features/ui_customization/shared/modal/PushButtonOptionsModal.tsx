...
export const PushButtonOptionsModal: React.FC<PushButtonOptionsModalProps> = ({
  ...
  const handleConfirmPage = () => {
    ...
    if (page < 0 || page >= pageCount) {
      Alert.alert('Error', `Page index must be between 0 and ${maxPageIndex}`);
      return;
    }
  ...
  const renderPageTab = () => (
    <View style={styles.body}>
      <Text style={styles.inputLabel}>Page Index (0 - {maxPageIndex})</Text>
      <TextInput
        style={styles.input}
        value={pageInput}
        onChangeText={value => setPageInput(value.replace(/[^0-9]/g, ''))}
        placeholder={`Enter page index (0 - ${maxPageIndex})`}
