...
const CPDFContentEditorExampleScreen = () => {
  ...
  useEffect(() => {
    if (pdfReaderRef) {
      pdfReaderRef.current?._editManager.historyManager.setOnHistoryStateChangedListener(
        (pageIndex, canUndo, canRedo) => {
          console.log(
            "ComPDFKit-RN: onContentEditorHistoryChanged - pageIndex:",
            pageIndex,
            " canUndo:",
            canUndo,
            " canRedo:",
  ...
  const handleMenuItemPress = async (action: string) => {
    switch (action) {
      ...
      case "Insert Text":
        ...
        break;
      case "Insert Image":
        // Example 1: Insert image from android assets
        ...
        // Example of adding an image from local file
        ...
        // const uri = 'content://media/external/images/media/12345'; // Replace with actual URI
        // if (uri) {
        //   const imageData = CPDFImageData.fromUri(uri);
        //   await pdfReaderRef.current?._pdfDocument.createNewImageArea({
        //     pageIndex: 0,
        //     offset: { x: 100, y: 400 },
        //     width: 300,
  ...
  const handleChangeEditType = async (editType: CPDFEditType[]) => {
    if (pdfReaderRef.current) {
      const result = await pdfReaderRef.current._editManager.changeEditType(
        editType
      );
