...
class ComPDFKit: NSObject, CPDFViewBaseControllerDelete, UIDocumentPickerDelegate{
  ...
  @objc(initWithPath: withResolver: withRejecter:)
  func initWithPath(licensePath: String, resolve: @escaping RCTPromiseResolveBlock, reject: @escaping RCTPromiseRejectBlock) {
    DispatchQueue.main.async {
      var licensePathToUse: String? = nil
      if FileManager.default.fileExists(atPath: licensePath) {
        licensePathToUse = licensePath
      } else {
        let fileName = (licensePath as NSString).deletingPathExtension
        let fileExtension = (licensePath as NSString).pathExtension
