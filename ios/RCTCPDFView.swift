...
class RCTCPDFView: UIView, CPDFViewBaseControllerDelete {
  ...
  private func createCPDFView() {
    ...
    configurations.defaultPageIndex = initPageIndex
    ...
    let bookmarkImageName = configurations.bookmarkImageName
    if !bookmarkImageName.isEmpty {
      if FileManager.default.fileExists(atPath: bookmarkImageName) {
        if let image = UIImage(contentsOfFile: bookmarkImageName) {
          configurations.bookmarkImage = image
        }
      } else {
    ...
    // selectTextLeftImage
    let selectTextLeftImageName = configurations.selectTextLeftImageName
    if !selectTextLeftImageName.isEmpty {
      if FileManager.default.fileExists(atPath: selectTextLeftImageName) {
        if let image = UIImage(contentsOfFile: selectTextLeftImageName) {
          configurations.selectTextLeftImage = image
        }
      } else {
    ...
    let selectTextRightImageName = configurations.selectTextRightImageName
    if !selectTextRightImageName.isEmpty {
      if FileManager.default.fileExists(atPath: selectTextRightImageName) {
        if let image = UIImage(contentsOfFile: selectTextRightImageName) {
          configurations.selectTextRightImage = image
        }
      } else {
    ...
    let rotationAnnotationImageName = configurations.rotationAnnotationImageName
    if !rotationAnnotationImageName.isEmpty {
      if FileManager.default.fileExists(atPath: rotationAnnotationImageName) {
        if let image = UIImage(contentsOfFile: rotationAnnotationImageName) {
          configurations.rotationAnnotationImage = image
        }
      } else {
  ...
  func setDisplayPageIndex(pageIndex : Int, rectList: [[String: Any]]) {
    if let pdfListView = self.pdfViewController?.pdfListView {
      ...
      if let squareAreaDict = self.pdfViewController?.configuration?.squareAreaDict {
        print("custom style\(squareAreaDict)")
          let sytle = CPDFSquareAreaStyle()
          if let fillHex = squareAreaDict["fillColor"] as? String {
              let c = ColorHelper.colorWithHexString(hex: fillHex)
              if let a = ColorHelper.alphaFromHex(fillHex) {
                  sytle.fillOpacity = a
              }
          ...
          if let borderHex = squareAreaDict["borderColor"] as? String {
              let c = ColorHelper.colorWithHexString(hex: borderHex)
              if let a = ColorHelper.alphaFromHex(borderHex) {
                sytle.borderOpacity = a
              }
              sytle.borderColor = c
          }

          if let w = squareAreaDict["borderWidth"] as? CGFloat {
              sytle.borderWidth = w
          }

          if let arr = squareAreaDict["borderDashPattern"] as? [NSNumber] {
              sytle.borderDashPattern = arr
          }
          
          pdfListView.setSquareAreas(areasDict, style: sytle)
      } else {
  ...
  func insertImagePage(pageIndex: Int, imagePath: URL, pageWidth: Float, pageHeight: Float, completionHandler: @escaping (Bool) -> Void) {
    if let pdfListView = self.pdfViewController?.pdfListView {
      var _index = pageIndex
      if pageIndex < 0 || pageIndex > pdfListView.document.pageCount {
        if pageIndex == -1 {
      ...
      let size = CGSize(width: Double(pageWidth), height: Double(pageHeight))
      let success = pdfListView.document.insertPage(size, withImage: imagePath.path, at: UInt(_index))
      self.pdfViewController?.pdfListView?.layoutDocumentView()
  ...
  func removePages(indexes: [Int], completionHandler: @escaping (Bool) -> Void) {
    if let pdfListView = self.pdfViewController?.pdfListView {
      var indexSet = IndexSet()
      for page in indexes {
        indexSet.insert(page)
  ...
  func movePage(fromIndex: Int, toIndex: Int, completionHandler: @escaping (Bool) -> Void) {
    if let pdfListView = self.pdfViewController?.pdfListView {
      let success = pdfListView.document.movePage(at: UInt(fromIndex), withPageAt: UInt(toIndex))
      completionHandler(success)
    }else {
      completionHandler(false)
    }
  ...
  func PDFViewBaseControllerAnndotationSelect(_ baseController: CPDFViewBaseController, forAnnotation annotation: CPDFAnnotation, isSelected: Bool) {
    ...
    let dict = pageUtil.getAnnotation(FormAnnotation: annotation)
    self.delegate?.onAnnotationSelectedChanged(self, annotationData: dict, isSelected: isSelected)
  }
  ...
  func PDFViewBaseControllerFormFieldAdded(_ baseController: CPDFViewBaseController, forFormField formField: CPDFWidgetAnnotation) {
    ...
    self.delegate?.onFormFieldAddChanged(self, formData: dict)
  }
  ...
  func PDFViewBaseControllerFormFieldSelect(_ baseController: CPDFViewBaseController, forFormField formField: CPDFWidgetAnnotation, isSelected: Bool) {
    ...
    let dict = pageUtil.getForm(FormAnnotation: formField)
    self.delegate?.onFormFieldSelectedChanged(self, formData: dict, isSelected: isSelected)
  }
  ...
  func PDFViewBaseControllerEditingAreaSelect(_ baseController: CPDFViewBaseController, forEditingArea editArea: CPDFEditArea, isSelected: Bool) {
    ...
    self.delegate?.onEditingAreaSelectedChanged(self, editingAreaData: dict, isSelected: isSelected)
  }
  ...
  func PDFViewBaseControllerAutoShowAnnotationPicker(_ baseController: CPDFViewBaseController, forAnnotationMode annotationMode: CPDFViewAnnotationMode, forAnnotation annotation: CPDFAnnotation?) {
    if annotationMode == .stamp {
      self.delegate?.onAutoShowAnnotationChanged(self, annotationData: ["type": "stamp"])
    } else if annotationMode == .signature {
      self.delegate?.onAutoShowAnnotationChanged(self, annotationData: ["type": "signature"])
    } else if annotationMode == .image {
      self.delegate?.onAutoShowAnnotationChanged(self, annotationData: ["type": "pictures"])
  ...
  func PDFViewBaseControllerHandleCustomMenuAction(_ baseController: CPDFViewBaseController, fronView view: Any, payload: [String : Any]) {
    if let CPDFAnnotation = payload["annotation"] as? CPDFAnnotation {
      ...
      self.delegate?.onCustomMenuActionChanged(self, payload: newPayload)
      return
      ...
      newPayload["image"] = data.base64EncodedString()
      self.delegate?.onCustomMenuActionChanged(self, payload: newPayload)
    } else {
      self.delegate?.onCustomMenuActionChanged(self, payload: payload)
    }
  }
  ...
  private var isPageIndexSet = false
  ...
  @objc func setPageIndex(_ pageIndex: Int) {
    self.pageIndex = pageIndex
    self.isPageIndexSet = true
    tryCreateCPDFViewIfReady()
  }
  
  private func tryCreateCPDFViewIfReady() {
    guard !hasCreatedView else { return }
    if isDocumentSet && isConfigurationSet && isPasswordSet && isPageIndexSet {
      hasCreatedView = true
      createCPDFView()
    }
  }
