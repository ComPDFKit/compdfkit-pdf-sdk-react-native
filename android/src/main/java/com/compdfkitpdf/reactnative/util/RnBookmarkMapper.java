...
/**
 * Provides bookmark mapping helpers for React Native bridge operations.
 */
public class RnBookmarkMapper {
          ...
          try {
            if (bookmark.getDate().startsWith("D:")) {
              CPDFDate createDate = CPDFDate.standardDateStr2LocalDate(
                bookmark.getDate());
              RnAppUtils.putLongCompat(map, "date", CDateUtil.transformToTimestamp(createDate));
            } else {
              ...
              if (date != null) {
                ...
                RnAppUtils.putLongCompat(map, "date", timestampMillis);
              }
            }
