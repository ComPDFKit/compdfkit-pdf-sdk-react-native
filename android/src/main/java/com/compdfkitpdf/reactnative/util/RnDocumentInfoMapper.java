...
/**
 * Provides document info mapping helpers for React Native bridge operations.
 */
public class RnDocumentInfoMapper {
  ...
  public static WritableMap getDocumentInfo(CPDFDocument document){
    ...
    try {
      ...
      long createDateTimes = RnAppUtils.toTimes(CDateUtil.transformPDFDate(info.getCreationDate()));
      if (createDateTimes != 0L){
        RnAppUtils.putLongCompat(documentInfo, "creationDate", createDateTimes);
      }
      long modificationDateTimes = RnAppUtils.toTimes(CDateUtil.transformPDFDate(info.getModificationDate()));
      if (modificationDateTimes != 0L){
        RnAppUtils.putLongCompat(documentInfo, "modificationDate", modificationDateTimes);
      }
      return documentInfo;
    } catch (Exception e) {
      return documentInfo;
