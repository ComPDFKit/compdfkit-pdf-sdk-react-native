...
/**
 * Maps signature field widget data between native objects and React Native maps.
 */
public class RnSignatureFieldWidgetCodec extends RnBaseWidgetCodec {
  ...
  public boolean addImageSignatures(Context context, CPDFAnnotation widget, String imagePath){
    ...
    try{
      String path = RnDocumentSourceResolver.getImportFilePath(context, imagePath);
  ...
  public CPDFWidget addWidget(CPDFDocument document, ReadableMap widgetMap) {
    ...
    if (widget.isValid()){
      ...
      if (TextUtils.isEmpty(title)){
        widget.setFieldName(RnAppUtils.getDefaultFiledName("Signature_"));
      } else {
