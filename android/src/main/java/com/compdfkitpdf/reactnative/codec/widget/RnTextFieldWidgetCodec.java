...
/**
 * Maps text field widget data between native objects and React Native maps.
 */
public class RnTextFieldWidgetCodec extends RnBaseWidgetCodec {
  ...
  public void covert(CPDFWidget widget, WritableMap map) {
    ...
    map.putString("fontColor", RnAppUtils.toHexColor(textWidget.getFontColor()));
    map.putDouble("fontSize", textWidget.getFontSize());
    map.putString("alignment", RnEnumConverter.textAlignmentToString(textWidget.getTextAlignment()));
  ...
  public void updateWidget(CPDFAnnotation annotation, ReadableMap annotMap) {
    ...
    textWidget.setTextAlignment(RnEnumConverter.stringToTextAlignment(alignment));
  ...
  public CPDFWidget addWidget(CPDFDocument document, ReadableMap widgetMap) {
    ...
    if (textWidget.isValid()){
      ...
      if (TextUtils.isEmpty(title)){
        textWidget.setFieldName(RnAppUtils.getDefaultFiledName("Text Field_"));
      } else {
