...
/**
 * Maps stamp annotation annotation data between native objects and React Native maps.
 */
public class RnStampAnnotationCodec extends RnBaseAnnotationCodec{
  ...
  public void covert(CPDFAnnotation annotation, WritableMap map) {
    ...
    if (stampAnnotation.isStampSignature()) {
      ...
      String stampTypeStr = RnEnumConverter.stampTypeToString(
      ...
      switch (stampAnnotation.getStampType()) {
        case STANDARD_STAMP:
          ...
          String standardStampStr = RnEnumConverter.standardStampToString(stampAnnotation.getStandardStamp());
        ...
        case TEXT_STAMP:
          ...
          textStampMap.putString("shape",
            RnEnumConverter.stampShapeToString(textStamp.getTextStampShape()));
          textStampMap.putString("color",
            RnEnumConverter.stampColorToString(textStamp.getTextStampColor()));
  ...
  public CPDFAnnotation addAnnotation(CPDFDocument document, ReadableMap annotMap) {
    ...
    if (stampAnnotation.isValid()) {
      ...
      CPDFStampAnnotation.StampType stampType = RnEnumConverter.stringToStampType(
        stampTypeStr);
      if (stampType == CPDFStampAnnotation.StampType.STANDARD_STAMP) {
        ...
        stampAnnotation.setStandardStamp(
          RnEnumConverter.stringToStandardStamp(standardStampStr));
        ...
        TextStampShape shape = RnEnumConverter.stringToStampShape(
          textStampMap.getString("shape"));
        CPDFStampAnnotation.TextStampColor color = RnEnumConverter.stringToStampColor(
  ...
  private Bitmap resolveImageBitmap(CPDFDocument document, ReadableMap annotMap) {
    ...
    return RnAppUtils.base64ToBitmap(base64Image);
  }

  /**
   * Resolves bitmap from image data.
   */
  private Bitmap resolveBitmapFromImageData(CPDFDocument document, ReadableMap imageDataMap) {
    ...
    switch (type) {
      case "filePath":
      case "uri": {
        String path = com.compdfkitpdf.reactnative.util.RnFileUtils.parseFilePath(document.getContext(), data);
        return TextUtils.isEmpty(path) ? null : BitmapFactory.decodeFile(path);
      }
      case "asset": {
        String path = com.compdfkitpdf.reactnative.util.RnFileUtils.assetToTempFile(document.getContext(), data);
        return TextUtils.isEmpty(path) ? null : BitmapFactory.decodeFile(path);
      }
      case "base64":
      default:
        return RnAppUtils.base64ToBitmap(data);
    }
  }
